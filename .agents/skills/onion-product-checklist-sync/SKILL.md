---
name: onion-product-checklist-sync
description: Sincronizar e monitorar checklists nativos do ClickUp com o sistema de desenvolvimento. Use quando o usuário quiser analisar a estrutura híbrida (texto + checklists nativos) de uma task ClickUp, calcular progresso real, detectar divergências entre texto e checklists, ou gerar relatórios de progresso/velocity. Aceita um task-id e flags --subtasks, --progress, --auto-sync.
disable-model-invocation: true
---

# 📋 ClickUp Checklist Sync - Análise e Monitoramento

O argumento fornecido é o `<task-id>` (e flags opcionais `--subtasks`, `--progress`, `--auto-sync`), lido do prompt do usuário.
...
```bash
/onion-product-checklist-sync <task-id>    # Análise completa de uma task
```

### **Monitoramento de Subtasks:**
```bash
/onion-product-checklist-sync <task-id> --subtasks    # Foco nas subtasks
```

### **Report de Progresso:**
```bash
/onion-product-checklist-sync <task-id> --progress    # Relatório detalhado
```

### **Sync Automático:**
```bash
/onion-product-checklist-sync <task-id> --auto-sync   # Atualiza comentários automaticamente
```
...
- **`/onion-product-task`**: Cria estrutura inicial (texto)
- **`/onion-engineer-start`**: Lê e analisa checklists durante início
- **`/onion-engineer-work`**: Monitora progresso durante desenvolvimento
- **`/onion-product-checklist-sync`**: Especialista em sincronização (ESTA skill)

### **Workflow Recomendado:**
```bash
# 1. Criar task com estrutura
/onion-product-task "Feature description"

# 2. [MANUAL] Criar checklists nativos no ClickUp

# 3. Sincronizar e analisar
/onion-product-checklist-sync <task-id>

# 4. Iniciar desenvolvimento com análise híbrida
/onion-engineer-start <feature-slug>

# 5. Monitorar progresso periodicamente
/onion-product-checklist-sync <task-id> --progress
```
...
- **Automatizar criação** de checklists durante /onion-product-task
...
```bash
# Task criada com /onion-product-task (apenas texto)
/onion-product-checklist-sync 86ac55kr8
...
/onion-product-checklist-sync 86ac55kr8 --progress
...
/onion-product-checklist-sync 86ac55kr8 --auto-sync
...
**Execute agora a análise de checklists para a task especificada (task_id fornecido no prompt do usuário).**
