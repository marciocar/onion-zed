---
name: onion-validate-test-strategy-create
description: Cria estratégias completas de teste baseadas no Framework de Testes. Use para gerar estratégias multi-perspectiva (White-box, Grey-box, Black-box) com cálculo automático de QA Story Points e estrutura de tasks no task manager.
...
Parâmetros (parsing no início): `feature-name` (obrigatório — nome da funcionalidade a ser testada), `risk-level` (opcional — baixo|médio|alto|crítico; default: médio), `complexity` (opcional — simples|médio|complexo|épico; default: médio), `task-manager` (opcional — clickup|asana|linear; usa TASK_MANAGER_PROVIDER se não fornecido), `project-id` (opcional — ID do projeto no task manager), `dry-run` (opcional — executa sem criar tasks reais).
...
**CRÍTICO:** Seguir padrão de `/onion-product-task`:
...
**None/Dry-run:** Salvar estrutura local em `.agents/onion/sessions/test-strategies/[feature-name].md`
...
3. Iniciar implementação: /onion-engineer-start [feature-slug]
...
**Checkout Flow:** `/onion-validate-test-strategy-create "checkout-flow" alto complexo --task-manager clickup --project-id 123456`  
→ ~13-14 QA points, Epic no ClickUp

**User Profile (Dry-run):** `/onion-validate-test-strategy-create "user-profile" médio simples --dry-run`  
→ ~5-6 QA points, relatório local apenas

**Payment Integration:** `/onion-validate-test-strategy-create "payment-integration" critico complexo --task-manager asana`  
...
- **Comandos relacionados:** `/onion-product-task`, `/onion-product-estimate`
- **Agentes relacionados:** delegue via tool `spawn_agent` para `.agents/onion/specialists/test-engineer.md` e `.agents/onion/specialists/test-planner.md`
