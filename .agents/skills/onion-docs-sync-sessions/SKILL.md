---
name: onion-docs-sync-sessions
description: Sincronizar e organizar sessões de trabalho do Sistema Onion. Use ao final de uma sessão significativa para preservar contexto, decisões e mudanças em `.agents/onion/sessions/`, gerando histórico navegável de atividades de desenvolvimento.
disable-model-invocation: true
---

Argumento opcional: nome customizado da sessão. Flags suportadas: `--archive`, `--validate-only`, `--force`, `--skip-git`, `--detailed`. Faça o parsing do nome e das flags a partir dos argumentos recebidos.
...
Este comando analisa o trabalho realizado na sessão atual, organiza a documentação gerada e sincroniza com a estrutura `.agents/onion/sessions/`, mantendo um histórico organizado de todas as atividades de desenvolvimento.
...
/onion-docs-sync-sessions

# Sincronizar com nome customizado
/onion-docs-sync-sessions "implementacao-feature-x"

# Sincronizar e arquivar sessão
/onion-docs-sync-sessions --archive

# Apenas validar sem sincronizar
/onion-docs-sync-sessions --validate-only
```
...
## Comandos Executados
1. `/onion-docs-build-tech-docs` - [Resultado]
2. `/onion-git-create-branch` - [Branch criada]
...
- **`.agents/onion/specialists/branch-documentation-writer.md`**: Gera documentação estruturada (delegue via tool `spawn_agent`)
- **`.agents/onion/specialists/metaspec-gate-keeper.md`**: Valida conformidade com padrões (delegue via tool `spawn_agent`)
- **`.agents/onion/specialists/gitflow-specialist.md`**: Auxilia em questões Git se necessário (delegue via tool `spawn_agent`)
...
/onion-docs-sync-sessions "refactoring-api" --archive

# Validar integridade sem modificar
/onion-docs-sync-sessions --validate-only

# Sincronização forçada com relatório detalhado
/onion-docs-sync-sessions --force --detailed
```
...
📁 Estrutura Criada:
  ✅ .agents/onion/sessions/2025-10-03_1030_dashboard-operacoes/
     ✅ README.md
     ✅ context.md
     ✅ decisions.md
     ✅ changes.md
...
🔗 Índice Atualizado:
  ✅ .agents/onion/sessions/INDEX.md

✅ Sessão sincronizada com sucesso!

📚 Para revisar: 
   cat .agents/onion/sessions/2025-10-03_1030_dashboard-operacoes/README.md
```
...
/onion-docs-sync-sessions "refactoring-contracts-module"
```

### Caso 2: Antes de Trocar de Branch
```bash
# Preservar contexto antes de mudar de tarefa
/onion-docs-sync-sessions --detailed
...
/onion-docs-sync-sessions --validate-only --detailed
```

### Caso 4: Arquivar Trabalho Concluído
```bash
# Mover sessão para archived após merge
/onion-docs-sync-sessions "feature-x-completed" --archive
```

## 🔗 Comandos Relacionados

- `/onion-docs-build-index` - Reconstrói índice de documentação
- `/onion-docs-docs-health` - Verifica saúde da documentação
- `/onion-docs-validate-docs` - Valida completude
- `/onion-git-help` - Ajuda com Git workflows
...
**Agente Responsável**: `.agents/onion/specialists/branch-documentation-writer.md`  
**Validador**: `.agents/onion/specialists/metaspec-gate-keeper.md`  
