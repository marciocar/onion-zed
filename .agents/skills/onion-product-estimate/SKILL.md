---
name: onion-product-estimate
description: Estimativa de Story Points. Use quando o usuário quiser estimar story points de uma tarefa ou feature, quebrar épicos, calibrar velocity do time, ou criar uma task já estimada. Orquestra o Framework de Story Points integrando análise de complexidade, decomposição e calibração contextual por senioridade.
disable-model-invocation: true
---

# 🎯 Estimativa de Story Points

Skill para orquestrar estimativas ágeis utilizando o Framework de Story Points, integrando análise de complexidade, decomposição de tarefas e calibração contextual.

Parâmetros (lidos do prompt do usuário): `task_description` (obrigatório), `assignee_level` (junior/pleno/senior, opcional), `methodology` (planning-poker/t-shirt/decomposition, opcional, default auto-detect), `create_task` (true para criar task com a estimativa, opcional).
...
Buscar (via grep) por métricas históricas disponíveis: "velocity tracking metrics historical data".
...
3. **Criar via Adapter:**
   - Usar adapter apropriado de `.agents/onion/utils/task-manager/adapters/`
...
### Com `/onion-product-task`
```markdown
Após estimar, criar task completa:
/onion-product-task "{{task_description}}" --story-points={{estimated_points}}
```

### Com `/onion-product-feature`
```markdown
Estimar feature completa antes de especificar:
/onion-product-estimate "{{feature_description}}" --create_task=true
/onion-product-feature "{{feature_description}}" --estimated_points={{points}}
```

### Com `/onion-product-spec`
```markdown
Incluir estimativa na especificação:
/onion-product-spec "{{feature}}" --include-estimate=true
```
...
```bash
/onion-product-estimate "Criar API REST para gerenciamento de usuários com autenticação JWT"
```
...
```bash
/onion-product-estimate "Implementar dashboard com múltiplas visualizações" --assignee_level=junior
```
...
```bash
/onion-product-estimate "Sistema completo de notificações com email, SMS e push"
```
...
```bash
/onion-product-estimate "Refatorar módulo de autenticação" --create_task=true --assignee_level=senior
```
...
- **Agente:** especialista story-points-framework (`.agents/onion/specialists/story-points-framework-specialist.md`)
- **Framework:** `docs/knowledge-base/frameworks/framework_story_points.md`
- **Skills relacionadas:** `/onion-product-task`, `/onion-product-feature`, `/onion-product-spec`
- **Agentes relacionados:** especialista product-agent, especialista task-specialist

## 📚 Base de Conhecimento

A skill utiliza o framework completo de story points disponível em:
