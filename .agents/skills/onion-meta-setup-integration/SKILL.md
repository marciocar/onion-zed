---
name: onion-meta-setup-integration
description: Configura integrações do Sistema Onion (Task Managers, Gamma, etc). Use para guiar a configuração segura de variáveis de ambiente para MCPs e APIs.
...
if [ -f .env.example ]; then
  ...
  echo "# Gerado por /onion-meta-setup-integration" >> .env
  echo "" >> .env
  echo "✅ .env criado"
fi
```
...
# Sugerir teste: /onion-product-task "Task de teste"
```
...
🚀 Próximos Passos:
∟ Execute /onion-product-task para criar sua primeira task
∟ Use delegação via tool `spawn_agent` lendo `.agents/onion/specialists/clickup-specialist.md` para operações ClickUp
∟ Ou execute /onion-engineer-start para iniciar desenvolvimento
...
- **Task Manager Abstraction**: `.agents/onion/utils/task-manager/README.md`
- **Detector de Provedor**: `.agents/onion/utils/task-manager/detector.md`
- **Documentação ClickUp**: `.agents/onion/docs/onion/clickup-integration.md`
- **Comando de Task**: `/onion-product-task` - Criar tasks com decomposição

## ⚠️ Notas Importantes

- **Task Manager é OBRIGATÓRIO** para comandos como `/onion-product-task` funcionarem com sincronização
