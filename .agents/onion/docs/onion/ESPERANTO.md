...
> O Claude Code é um IDE com IA integrada baseado no VS Code. Os comandos customizados (`.agents/skills/`) e agentes (`.agents/onion/specialists/`) são extensões do Sistema Onion e não recursos nativos do Claude Code. Use `@Docs`, `@Web` e `@Files` como símbolos nativos do Claude Code para contexto adicional.
...
1. **Localização**: Gravar em `.agents/onion/sessions/[session-name]/temp/`
...
```
.agents/onion/sessions/feat-user-auth/
...
- Use `@Files .agents/onion/sessions/[session-name]/` para contexto de sessão no Claude Code
...
- [Claude Code Documentation](https://docs.claude.com/en/docs/claude-code/overview) - Guia completo em português
- [Working with Documentation](https://docs.claude.com/en/docs/claude-code/memory) - Integração de docs
- [Models Guide](https://docs.claude.com/en/docs/about-claude/models) - Modelos de IA disponíveis
