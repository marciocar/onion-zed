...
location: '.agents/skills/language-standards/language-and-documentation.mdc'
...
version: '1.0'
// Propósito: Framework de skills .agents/ e specialists
// Componentes: 46 comandos, 14+ agentes, ClickUp MCP integration
```

### 🎯 Contexto do Projeto
```markdown
Sistema Onion - Framework avançado de skills .agents/
...
```typescript
structure: {
  commands: '.agents/skills/',      // Organizados por categoria
  agents: '.agents/onion/specialists/',          // YAML header + Markdown
  sessions: '.agents/onion/sessions/<feature>/', // Contexto de desenvolvimento
...
```typescript
optimization: {
  ignore: 'Minimizar arquivos irrelevantes com .claudeignore',
...
// TEMPLATE: '.agents/onion/prompts/clickup-patterns.md' - seção DESCRIPTIONS
```
...
// TEMPLATE: '.agents/onion/prompts/clickup-patterns.md' - seção COMMENTS
...
```
.claudeignore              # Otimização de contexto
AGENTS.md               # Regras do Claude Code (este arquivo)
...
2. **Workspace Rules** - Regras do workspace (.agents/skills/language-standards/)
