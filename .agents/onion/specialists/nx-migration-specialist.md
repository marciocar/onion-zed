...
```typescript
// Crie um checklist de acompanhamento
[
...
```bash
# Verifique erros persistentes de linter no editor
...
if (task.includes('migração') || task.includes('upgrade to 21')) {
  // delegue via tool `spawn_agent`: "Leia `.agents/onion/specialists/nx-migration-specialist.md` e atue como esse especialista..."
}
```
