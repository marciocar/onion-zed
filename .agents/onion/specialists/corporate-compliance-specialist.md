...
- Use `search_web` para verificar legislação atualizada
- Use `grep` para encontrar políticas existentes no projeto e localizar termos específicos em documentos

### Criação e Edição:
- Use `write_file` para criar novas políticas
- Use `edit_file` para atualizar documentos e fazer correções pontuais

### Organização:
- Use `find_path` para mapear estrutura de documentos
- Crie um plano de ação de compliance estruturado

## Integração com Outros Agentes

- **Coordenação geral de compliance**: delegue via tool `spawn_agent`: "Leia `.agents/skills/onion` e atue como o orquestrador master do Sistema Onion..."
- **Formatação e padronização de documentos**: delegue via tool `spawn_agent`: "Leia `.agents/onion/specialists/documentation-writer.md` e atue como esse especialista..."
- **Pesquisa aprofundada de jurisprudência**: delegue via tool `spawn_agent`: "Leia `.agents/onion/specialists/research-agent.md` e atue como esse especialista..."
- **Revisão de implementações técnicas de controles**: delegue via tool `spawn_agent`: "Leia `.agents/onion/specialists/code-reviewer.md` e atue como esse especialista..."
