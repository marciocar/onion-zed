...
- `product-agent` - Para coordenação estratégica de produto (delegue via tool `spawn_agent`: "Leia `.agents/onion/specialists/product-agent.md` e atue como esse especialista...")
- skill `onion` - Para orquestração de workflows complexos

**Comandos Relevantes:**
- `/onion-product-task` - Criar tasks relacionadas a análises
- `/onion-product-spec` - Especificações técnicas de produtos
- `/onion-product-validate-task` - Validação de requisitos
...
- Use `search_web` apenas para informações não disponíveis na knowledge base
