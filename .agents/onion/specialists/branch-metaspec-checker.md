...
> constituição de validação — delegue via tool `spawn_agent`: "Leia
> `.agents/onion/specialists/metaspec-gate-keeper.md` e atue como esse
> especialista..."), porém **focado no diff do branch atual** (versão
> leve, pré-PR). Para validação profunda de um artefato específico, use a skill
> `/onion-meta-metaspec-validate`. Descubra as metaspecs dinamicamente (`docs/meta-specs/`)
