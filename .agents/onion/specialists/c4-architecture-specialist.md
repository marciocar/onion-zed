...
- **🤝 Integração**: Bridge automática com o mermaid-specialist para validação (delegue via tool `spawn_agent`: "Leia `.agents/onion/specialists/mermaid-specialist.md` e atue como esse especialista...")
...
```typescript
// Implementação conceitual - executada via ferramentas do Zed
...
interface TemplateSelector {
  selectTemplate(projectType: ProjectType, confidence: number): TemplateConfig {
    ...
    const templates = {
      ...
      // ... outros templates de .agents/onion/utils/c4-templates.md
    };
...
interface ComponentDiagramGenerator {
  ...
  private async analyzeFileStructure(path: string): Promise<FileStructure> {
    // Usa find_path para encontrar arquivos TypeScript/JavaScript
...
Para validação, otimização e correção de erros, delegue via tool `spawn_agent`: "Leia `.agents/onion/specialists/mermaid-specialist.md` e atue como esse especialista...".
...
interface MermaidSpecialistBridge {
  ...
  async validateGeneratedDiagram(mermaidCode: string): Promise<ValidationResult> {
    // Delega automaticamente para o mermaid-specialist para validação de sintaxe
  ...
  async fixDiagramErrors(mermaidCode: string, errors: string[]): Promise<string> {
    // Delega correção de erros para o mermaid-specialist
    return await this.delegateToMermaidSpecialist(
...
Para gerar documentação textual completa, delegue via tool `spawn_agent`: "Leia `.agents/onion/specialists/c4-documentation-specialist.md` e atue como esse especialista...".
...
interface C4DocumentationBridge {
  ...
  async generateCompleteC4Analysis(projectPath: string, options: AnalysisOptions): Promise<UnifiedC4Output> {
    ...
    if (needsDocumentation) {
      // Passo 5: Delega automaticamente para o c4-documentation-specialist (Slave)
  ...
  async delegateToDocumentationSpecialist(cacheId: string, options: AnalysisOptions): Promise<Documentation> {
    ...
    // Invoca automaticamente o c4-documentation-specialist com análise em cache
...
```bash
# Mermaid Validation (delegue ao mermaid-specialist via spawn_agent)
...
# These trigger automatic delegation ao c4-documentation-specialist via spawn_agent:
...
```typescript
// Loads detection rules from .agents/onion/utils/c4-detection-rules.md
...
### **Template Loading from .agents/onion/utils/**
```typescript
class TemplateEngine {
  private templates = this.loadTemplates();
  
  private loadTemplates(): TemplateMap {
    // Load templates from .agents/onion/utils/c4-templates.md
...
6. Delegate ao mermaid-specialist (via spawn_agent) for validation
...
5. [SLAVE] Auto-delegate ao c4-documentation-specialist (via spawn_agent)
...
3. Automatically delegate ao c4-documentation-specialist (via spawn_agent)
...
5. Other (specify)
  ...
  async handleMermaidGenerationError(error: string): Promise<string> {
    // Delegate error fixing ao mermaid-specialist (via spawn_agent)
    return await this.delegateToMermaidSpecialist(
...
- **Meta-agent delegation**: o orquestrador `onion` pode delegar automaticamente para o c4-architecture-specialist
- **Command integration**: Comandos especializados em `/onion-architect-*`
...
- `find_path` - Discover project structure / find files by patterns
- `grep` - Search for patterns and dependencies
- delegação ao mermaid-specialist (via `spawn_agent`) - Mermaid validation and optimization
- Template access via `.agents/onion/utils/c4-templates.md`
- Detection rules via `.agents/onion/utils/c4-detection-rules.md`
- Mermaid patterns via `.agents/onion/utils/c4-mermaid-patterns.md`
- delegação ao c4-documentation-specialist (via `spawn_agent`) - Master-slave coordination for complete C4 analysis
