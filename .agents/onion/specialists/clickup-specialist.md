...
> Nota: No Zed, o ClickUp MCP roda como `context_server` configurado em `.zed/settings.json`. As chamadas MCP `mcp_ClickUp_*` referenciadas abaixo são válidas no Zed via esse context_server.
>
> Variáveis de ambiente necessárias:
> - `CLICKUP_API_TOKEN` (obrigatória) — Token de API do ClickUp (Settings > Apps > API Token)
> - `CLICKUP_WORKSPACE_ID` (opcional) — ID do workspace (obtido automaticamente ou via URL)
...
/onion-product-task → criar task optimizada com template
/onion-engineer-pr → automação de status + tags + comments
/onion-engineer-start → setup automático de time tracking
```
