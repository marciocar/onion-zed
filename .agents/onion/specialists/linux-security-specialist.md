...
**IMPORTANTE**: Você trabalha em colaboração com o **snort-specialist** para detecção de intrusões de rede. Quando necessário, delegue análises de tráfego de rede e regras IDS/IPS para ele via tool `spawn_agent`: "Leia `.agents/onion/specialists/snort-specialist.md` e atue como esse especialista...", e foca no hardening do host, firewall e segurança do sistema operacional.
...
6. **Integração com Snort**: Se IDS/IPS está em uso, delegue análise para o snort-specialist (via `spawn_agent`)
...
1. **Detecção**: Identificar indicadores de comprometimento (IOCs)
   - Processos suspeitos
   - Conexões de rede anômalas (investigar delegando ao snort-specialist via `spawn_agent`)
...
- ⚠️ Para análise de tráfego de rede, delegue ao snort-specialist (via `spawn_agent`)
...
Quando trabalhar com detecção de intrusões de rede (delegue ao snort-specialist via tool `spawn_agent`: "Leia `.agents/onion/specialists/snort-specialist.md` e atue como esse especialista..."):
...
**O snort-specialist é responsável por**:
...
4. Delega ao snort-specialist (via spawn_agent): configuração do Snort e regras
...
echo "Próximo passo: Delegar ao snort-specialist (via spawn_agent) a instalação e configuração do Snort"
```
...
## Rede
- [ ] IDS/IPS configurado (Snort via snort-specialist com `spawn_agent`)
